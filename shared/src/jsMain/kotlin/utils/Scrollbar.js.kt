...
/**
 * My Custom Implementation: rememberScrollbarAdapter for LazyGridState
 */
@Composable
fun rememberScrollbarAdapter(
...
fun ScrollbarAdapter(
    scrollState: LazyGridState
): ScrollbarAdapter = LazyGridScrollbarAdapter(
    scrollState
)

private class LazyGridScrollbarAdapter(
...
//        AppLogger.d(offset)
    ...
    private val averageItemSize by derivedStateOf {
        scrollState
            ...
            .visibleItemsInfo
            .asSequence()
            //.map { it.size.height } //TODO, Scroll is not in alignment with mouse position
            .map { it.size.height - it.size.height * 0.6 } //simple workaround as of now, Scroll is not in alignment with mouse position
            .average()
            .toFloat()
    }
}
